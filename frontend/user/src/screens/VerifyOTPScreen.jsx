...
const VerifyOTPScreen = ({ phoneNumber, onBack, onSuccess }) => {
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            // Since unwrap() succeeded, it's a success. 
            // result in authSlice is { token, user }
            if (result) {
    ...
    const handleResend = async () => {
        ...
        try {
            const result = await dispatch(sendOtp({ phone: phoneNumber })).unwrap();
            setResending(false);

            // result is the API data. If unwrap didn't throw, it's likely a success.
            setTimer(59);
