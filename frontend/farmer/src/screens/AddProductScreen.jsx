...
const AddProductScreen = ({ onBack, onSave }) => {
    ...
    const fetchCategories = async () => {
        try {
            ...
            // Extract the array properly, handling pagination (results)
            ...
            if (categoriesData && Array.isArray(categoriesData)) {
                const vegetableCats = categoriesData.filter(cat => cat.category_type === 'vegetable');
    ...
    const handleListProduct = async () => {
        if (categories.length === 0) {
            Alert.alert('Error', 'Category type must be added. Please wait until categories are available.');
            return;
        }
