...
const LoginScreen = ({ onBack, onSignup, onContinue, role, onLoginSuccess }) => {
    ...
    const { sendOtp, setAuthData, loading, error } = useAuth();
    ...
    useEffect(() => {
        ...
        // 🔴 NEW: Request FCM permission on app load
    ...
    const requestFCMPermission = async () => {
        try {
            ...
            if (enabled) {
                ...
                // Get FCM token and store it (but don't send yet)
                const token = await messaging().getToken();
                await AsyncStorage.setItem('fcmToken', token);
                console.log('✅ FCM Token stored:', token.substring(0, 50) + '...');
            } else {
    ...
    const saveFCMTokenToBackend = async (accessToken) => {
        try {
            ...
            const response = await fetch(API_URLS.FCM_TOKEN || `${API_URLS.BASE_URL}/delivery/fcm-token/`, {
                method: 'POST',
                headers: {
            ...
            // Don't block login even if FCM save fails
            return false;
        }
    };
    ...
    const handleSendOTP = async () => {
        ...
        if (!phoneRegex.test(trimmedPhone)) {
            Alert.alert('Error', 'Please enter a valid 10-digit Indian phone number (starts with 6-9)');
            return;
        }

        const result = await sendOtp(trimmedPhone, countryCode);
        console.log("OTP Response:", result);
        if (result.success) {
            if (result.data.role === "delivery") {
                Alert.alert('Success', 'OTP sent successfully!');
                setTimeout(() => {
                    onContinue(trimmedPhone, role);
                }, 300);
            } else {
    ...
    const handleGoogleLogin = async () => {
        try {
            ...
            const response = await fetch(API_URLS.GOOGLE_LOGIN, {
                ...
                body: JSON.stringify({
                    token: idToken,
                    role: role // Send role too if needed
                }),
            });
            ...
            if (response.ok) {
                ...
                // 🔴 FIX: Update AuthContext directly so the app knows you are logged in
                const accessToken = data.access_token || data.access;
                
                // Keep setting accessToken in AsyncStorage if other parts of the app use it
                await AsyncStorage.setItem('accessToken', accessToken);
                
                // This updates the main App state so it can navigate to the Home screen
                await setAuthData(accessToken, data.user);

                // 🔴 NEW: Save FCM token to backend
                await saveFCMTokenToBackend(accessToken);
